...
def WangGoo(stock_id):
	url = "https://www.wantgoo.com/stock/" + stock_id + "/institutional-investors/trend-data?topdays=10"
	headers = {
		'user-agent': 'Mozilla/4.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/84.0.4147.105 Safari/537.36'
	}
	...
	count = 0
	for item in data:
		sumForeign = item['sumForeignNoDealer'] + item['sumForeignWithDealer']
		sumIng = item['sumING']
		sumDealer = item['sumDealerBySelf'] + item['sumDealerHedging']
		if(foreign_sell == -1 and sumForeign > 0):
			foreign_sell = count
		
		elif(foreign_buy == -1 and sumForeign < 0):
			foreign_buy = count
		
		if(ing_sell == -1 and sumIng > 0):
			ing_sell = count
		
		elif(ing_buy == -1 and sumIng < 0):
			ing_buy = count
		
		if(dealer_sell == -1 and sumDealer > 0):
			dealer_sell = count
		
		elif(dealer_buy == -1 and sumDealer < 0):
			dealer_buy = count
		
		count += 1

	if foreign_sell > foreign_buy:
		foreign = -foreign_sell
	else:
		foreign = foreign_buy
